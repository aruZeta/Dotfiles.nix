...
#+NAME: python.overlays
#+begin_src nix
nixpkgs.overlays = [
  (
    <<python.overlays.apsw-fix>>
  )
];
#+end_src
...
in {
  calibre = overlayPrev.calibre.override { python3Packages = python'.pkgs; };
}
#+end_src
...
home.packages = with pkgs; [
  (
    <<sway.scripts.sway-screenshot>>
  )

  (
    <<sway.scripts.sway-exit>>
  )
];
#+end_src
...
#+NAME: sway.scripts.sway-screenshot
#+begin_src nix
...
home.packages = with pkgs; [
  (
    <<user.scripts.apply-system>>
  )

  (
    <<user.scripts.apply-user>>
  )

  (
    <<user.scripts.update-system>>
  )

  (
    <<user.scripts.update-user>>
  )
];
#+end_src
...
#+NAME: user.scripts.apply-system
#+begin_src nix
...
#+NAME: user.scripts.apply-user
#+begin_src nix
writeShellScriptBin "apply-user"
  ''
  pushd ${config.xdg.userDirs.extraConfig.XDG_DOTFILES_DIR}
...
#+NAME: user.scripts.update-system
#+begin_src nix
