...
(defun aru/web-server-open ()
  (interactive)
  (let ((name (concat "web-server-" (projectile-project-name))))
    (if (get-process name)
        (message "Web server already started in this project.")
      (start-process-shell-command name name "python3 -m http.server"))))

(defun aru/web-server-close ()
  (interactive)
  (let* ((name (concat "web-server-" (projectile-project-name)))
...
in {
  ...
  "${mod}+Shift+s" = "exec grim -g \"$(slurp)\" -t png - | swappy -f -";
  "${mod}+Shift+w" = "kill";
